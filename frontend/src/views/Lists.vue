    ...
    <section class="lists">
      ...
      <header class="columns">
        <div class="column is-2">
          <b-field :label="$t('globals.terms.listType')" label-position="on-border">
            <b-select
              :placeholder="$t('globals.terms.listType')"
              expanded
              v-model="queryParams.list_type"
            >
              <option value="default" key="default">Default</option>
              <option value="smart" key="smart">Smart</option>
              <option value="fltrd" key="fltrd">Filtered</option>
            </b-select>
          </b-field>
        </div>
        <div class="column is-2">
          <b-field>
            <b-button @click="getLitsByType" type="is-primary" icon-left="file-find-outline">
              Filter
            </b-button>
          </b-field>
        </div>
        ...
        >
          <div>
            ...
            <b-tag :data-cy="`optin-${props.row.optin}`">
              <b-icon
                :icon="
                  props.row.optin === 'double' ? 'account-check-outline' : 'account-off-outline'
                "
                size="is-small"
              />
              {{ " " }}
            ...
            >{{ " " }}
            <a
              v-if="props.row.optin === 'double'"
              class="is-size-7 send-optin"
              href="#"
              @click="$utils.confirm(null, () => createOptinCampaign(props.row))"
...
export default Vue.extend({
  ...
  methods: {
    ...
    getLists() {
      this.$api.getLists({
        list_type: this.queryParams.list_type,
    ...
    deleteList(list) {
      this.$utils.confirm(this.$t("lists.confirmDelete"), () => {
