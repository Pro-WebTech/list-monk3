        ...
        <div class="column is-8">
          <p v-if="isEditing" class="tags">
            ...
            <span v-if="isEditing" class="has-text-grey-light is-size-7">
              {{ $t("globals.fields.id") }}: {{ data.id }} / {{ $t("globals.fields.uuid") }}:
              {{ data.uuid }}
            </span>
          </p>
                ...
                <form @submit.prevent="onSubmit">
                  ...
                  <div class="columns">
                    <div class="column is-4">
                      <b-field :label="$t('campaigns.criterion')" label-position="on-border">
                        <b-select
                          :placeholder="$t('campaigns.criterion')"
                          expanded
                          v-model="form.criterion"
                          :disabled="!canEdit"
                        >
                          <option value="include" key="include">Include</option>
                    ...
                    <div class="column is-4">
                      <!---->
                      <b-field :label="$t('campaigns.eventType')" label-position="on-border">
                        <b-select
                          :placeholder="$t('campaigns.eventType')"
                          expanded
                          v-model="form.eventType"
                          :disabled="!canEdit"
                        >
                          <option value="opened" key="opened">Opened</option>
                          <option value="clicked" key="clicked">Clicked</option>
                          <option value="emailed" key="emailed">Emailed</option>
                        </b-select>
                      </b-field>
                    </div>
                    <div class="column is-4">
                      <b-field label="Lists" label-position="on-border">
                        <b-input
                          v-model="form.listRange"
                          :disabled="!canEdit"
                          :placeholder="$t('campaigns.listRange')"
                        ></b-input>
                      </b-field>
                    </div>
                  </div>
                  <div class="columns">
                    <div class="column is-4">
                      <b-field label="Campaigns" label-position="on-border">
                        <b-input
                          v-model="form.listCampaignIDRange"
                          :disabled="!canEdit"
                          :placeholder="$t('campaigns.listCampaignIDRange')"
                        ></b-input>
                      </b-field>
                    </div>
                    <div class="column is-3">
                      <b-field label="Max Subscribers" label-position="on-border">
                        <b-input
                          v-model="form.maxSubscribers"
                          :disabled="!canEdit"
                          :placeholder="$t('campaigns.maxSubscribers')"
                        ></b-input>
                      </b-field>
                    </div>
                    <div class="column is-3">
                      <b-field label="Time Range" label-position="on-border">
                        <b-input
                          ...
                          :disabled="!canEdit"
                          :placeholder="$t('campaigns.timeRange')"
                        ></b-input>
                      </b-field>
                    </div>
                  ...
                  <b-field :label="$tc('globals.terms.messenger')" label-position="on-border">
                    <b-select
                      :placeholder="$tc('globals.terms.messenger')"
...
export default Vue.extend({
  ...
  methods: {
    ...
    async timeRangeIconClicked() {
      this.isLoading = true;
      this.$api
        .filterSubscribers({
          selection: this.form.criterion,
          type: this.form.eventType,
          maxsubscribers: this.form.maxSubscribers || 10000000,
          timerange: this.form.timeRange,
          listrange: this.form.listRange,
          listCampaignIDRange: this.form.listCampaignIDRange
        })
        .then(response => {
          this.$utils.toast(
            `${response.subscriber_count || 0} ${this.$t("campaigns.filterMessage")}`
          );
