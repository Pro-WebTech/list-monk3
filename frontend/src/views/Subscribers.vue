    ...
    <section class="subscribers">
      <header class="columns">
        <div class="column is-half">
          <h1 class="title is-4">
            ...
            <span v-if="currentList"> &raquo; {{ currentList.name }} </span>
          </h1>
        </div>
      ...
      <b-modal scroll="keep" :aria-modal="true" :active.sync="isBulkListFormVisible" :width="450">
        <subscriber-bulk-list
          :numSubscribers="this.numSelectedSubscribers"
...
export default Vue.extend({
  ...
  methods: {
    ...
    exportSubscribers() {
      this.$utils.confirm(
        this.$t("subscribers.confirmExport", { num: this.subscribers.total }),
        () => {
      ...
      if (!this.bulk.all && this.bulk.checked.length > 0) {
        ...
        fn = () => {
          ...
          this.$api.deleteSubscribers({ id: ids }).then(() => {
            ...
            this.$utils.toast(
              this.$t("subscribers.subscribersDeleted", { num: this.numSelectedSubscribers })
            );
          });
        };
      } else {
