...
func handleSubscriptionPage(c echo.Context) error {
	...
	if unsub {
		...
		var subcribersRes models.Subscribers
		if err := app.queries.GetSubscriber.Select(&subcribersRes, 0, subUUID, ""); err != nil {
			app.log.Printf("error unsubscribing - fetching subscriber: %v", err)
			return c.Render(http.StatusInternalServerError, tplMessage,
				makeMsgTpl(app.i18n.T("public.errorTitle"), "",
					app.i18n.Ts("public.errorProcessingRequest")))
		}

		if _, err := app.db.Exec(fmt.Sprintf(app.queries.InsertAttributeBlocklistSubscribers, fmt.Sprintf("(%d, '%s', '%s', '%s', %d)", subcribersRes[0].ID, "Unsubscribed", "The subscriber has unsubscribed from the list.", time.Now().Format("2006-01-02T15:04:05Z"), 0))); err != nil {
			app.log.Printf("error unsubscribing updating events: %v", err)
			return c.Render(http.StatusInternalServerError, tplMessage,
				makeMsgTpl(app.i18n.T("public.errorTitle"), "",
					app.i18n.Ts("public.errorProcessingRequest")))
		}
...
func handleLinkRedirect(c echo.Context) error {
	...
	var url string
	if _, err := app.queries.UpdateLastEmailClicked.Exec(subUUID); err != nil {
		app.log.Printf("error update subscribers - last email open: %s", err)
	}
	...
	if len(email) > 0 {
		url = fmt.Sprint(url, email)
	}

	app.log.Println("redirect URL: ", url)

	return c.Redirect(http.StatusTemporaryRedirect, url)
}

func handleLinkCheck(c echo.Context) error {
...
func handleRegisterCampaignView(c echo.Context) error {
	...
	if campUUID != dummyUUID && subUUID != dummyUUID {
		if _, err := app.queries.RegisterCampaignView.Exec(campUUID, subUUID); err != nil {
			app.log.Printf("error registering campaign view uuid %s subuuid %s: %s", campUUID, subUUID, err)
		}
		if _, err := app.queries.UpdateLastEmailOpen.Exec(subUUID); err != nil {
			app.log.Printf("error update subscribers - last email open subuuid %s: %s", subUUID, err)
		}
	}
...
func handleSelfExportSubscriberData(c echo.Context) error {
	...
	}, app.manager.Cfg.Concurrency); err != nil {
