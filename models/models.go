...
var regTplFuncs = []regTplFunc{
	regTplFunc{
		...
		replace: `{{ TrackLink "$6" "$3" . }}`,
	},
	regTplFunc{
		regExp:  regexp.MustCompile("{{(\\s+)?TrackView(\\s+)?(.+?)(\\s+)?}}"),
		replace: `{{ TrackView "$3" . }}`,
	},
	regTplFunc{
		regExp:  regexp.MustCompile(`{{(\s+)?(UnsubscribeURL|OptinURL|MessageURL)(\s+)?}}`),
		replace: `{{ $2 . }}`,
	},
}
...
type Base struct {
	...
	LastEmailSent    null.Time `db:"last_email_sent" json:"last_email_sent"`
	LastEmailOpen    null.Time `db:"last_email_open" json:"last_email_open"`
	LastEmailClicked null.Time `db:"last_email_clicked" json:"last_email_clicked"`
}
...
type Event struct {
	...
	EventReason    string    `db:"event_reason" json:"eventReason"`
}
...
type CampaignMeta struct {
	...
	SentPercentage int `db:"sent_percentage" json:"sent_percentage"`

	StartedAt    null.Time `db:"started_at" json:"started_at"`
	ToSend       int       `db:"to_send" json:"to_send"`
	Sent         int       `db:"sent" json:"sent"`
	TotalSent    int       `db:"total_send" json:"total_send"`
	EmailAllowed int       `db:"email_allowed" json:"email_allowed"`
}
